...
try {
  ...
  if (args.dt) {
    ...
    let filename =
      typeof args.file === "boolean" || args.file === undefined
        ? name + ".d.ts"
        : args.file;
    ...
    if (!args.overwrite && fs.existsSync(filename)) {
      console.error(
        `File ${filename} already exists and -overwrite was not specified; exiting.`
      );
      process.exit(2);
    }
  ...
  if (e instanceof ArgsError) {
    ...
    console.log(
      `Couldn't load module "${args.module}". ` +
        `Please install it globally (npm install -g ${
          args.module
        }) and try again.`
    );
    process.exit(1);
